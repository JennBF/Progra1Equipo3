...
void Clientes::guardarEnArchivos(const std::vector<Clientes>& lista) {
    // ------------------- Parte 1: Guardado binario -------------------
    ...
    // ------------------- Parte 2: Guardado tabular -------------------
    std::ofstream archivoTxt("Clientes.tmp");
    if (!archivoTxt) {
        std::cerr << "\n\tError al abrir Clientes.tmp para escritura.\n";
        return;
    }

    // Encabezado
    archivoTxt << "---------------------------------- Lista de Clientes ----------------------------------\n";
    archivoTxt << std::left
               << std::setw(10) << "ID"
               << std::setw(25) << "Nombre"
    ...
    // Reemplazo seguro del archivo .txt anterior
